...
| 目标模型（北极星）设计 | 草稿 v1 已写，已拍板 15 项；**已抽 ADR 0011–0017**（价值公式 / 证据结论分离 / 当前状态派生 / 置信度两轴 / scope-owner 提权 / 采集四档 / Entity-主题 P2）；待拍板 3 项（迁移路径 / workbench / 实体属性文档） | [v1](designs/target-model/v1.md) · [LATEST](designs/target-model/LATEST.md) |
...
- 讨论中 topic（2026-08-14 重定位；已收敛到 [目标模型](designs/target-model/v1.md)（北极星）作为唯一裁判，S0–S5 降为它的下辖挂点）：
  - **目标模型**（草稿 v1）：北极星=价值公式；Evidence 唯一不可再生（用户纠正=特权 Evidence）→ Claim/Edge 派生可重算；推理在边；演变只做推导记录。已抽 ADR 0011–0017；待拍板 3 项（渐进迁移路径 / workbench / 实体属性文档）
  - **S-pre 遥测基座** = Evidence 采集管线（插件信号面已盘点：[10-plugin-signal-surface](notes/research/2026-08-14-reuse-feedback-signals/10-plugin-signal-surface.md)）
  - S0 = Claim 属性（6 维度，置信度已拆两轴）；S1 = Edge 的 refines/generalizes（scope 提权）；S2 = 两轴置信度；S3 = 对账触发 + workbench 裁决；S4 = scope/owner 提权；S5 = 状态查询召回；Entity/主题 = P2 附属
