...
8. **M3 前置文档：迁移脚本设计 v1**（接口/幂等/断点续传/回放，[migration-path](initiatives/crystal/migration-path.md) Stage C）——落稿后进入 M3 开发（memories → evidence 一次性全量迁移 + 对账重生成 claim）。
9. M4 前置：插件切换契约（四端各自接入/回退）→ M5 前置：退役检查单。
10. 需求层已有 [crystal PRD](initiatives/crystal/prd.md)（用户故事 + 能力验收 A1–A11），各 design v1 已把对应验收细化。
