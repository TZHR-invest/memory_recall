...
| dsh 客户端插件（memory-recall-dsh） | 已完成（2026-08-14：5 工具 + 自动召回注入 + 自动捕获；17 测试全绿；headless 真实运行时 E2E 验证通过；已安装到 ~/.dsh web profile 并生效；修复 MR-022 补平台元数据、MR-023 补浏览器端 __ModuleLoader__ 注册，dsh web 正常） | [dsh 插件](../apps/api/src/plugins/dsh/README.md) |
