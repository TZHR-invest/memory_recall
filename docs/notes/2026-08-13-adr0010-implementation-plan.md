...
> 类型: 实施计划 · 日期: 2026-08-13 · 状态: **暂停**（阶段 1 已完成，阶段 2/3 暂停）
...
- **暂停执行**：阶段 1 已完成（四端插件移除文档功能，opencode 1.9.0）；阶段 2（用户升级）与阶段 3（后端删除）暂停。
- 原因：待 v2 目标模型落地后，文档系统（`documents`/`chunks`/`chunk_entities` + `/documents*`）随重构**自然废弃**，无需单独再走一次三阶段删除。
- 恢复条件：若 v2 落地前出现文档功能回归或误用，再回来继续阶段 2/3。
