...
**定位**：不是"找提升"，而是**排除静默退化**——图谱通道承担 26% 注入项，而这次改动会让候选池从
"半个实体"变成"整个实体"，多出的候选**可能挤掉原本入选的条目**（已知机制），这只能测、不能看。

**设计**：40 条**真实 query**（从 `recall_traces` 取、跨 4 个容器分层）+ 两臂（ON/OFF）+ **对照臂**
（同配置连跑两次）；裁判 `commandcode/Qwen/Qwen3.7-Max` 盲评，**逐条 1–5 分**（洗牌 + 马甲标签，
裁判不知道条目属于哪一臂）；判据**预注册**在脚本 docstring 里；脚本
`apps/api/scripts/ab_entity_family_inferiority.py`（可复跑，产物落 `apps/api/backups/ab-entity-family/`）。

| 指标 | 结果 |
|---|---|
| **对照臂（同配置两次）** | **40/40 完全一致** ⇒ **测量噪声为 0**（确定性的额外红利：符号计数这次可解释，不像上次 low-vs-low 也能判出 9/10 胜者） |
| 只在 ON 出现的条目 | 63 条，均分 **1.95** |
| 只在 OFF 出现的条目 | 56 条，均分 **1.96** |
| **Δ（主指标）** | **−0.012**，按 query 聚类 bootstrap 95% CI **−0.40 ~ 0.44** ⇒ 两侧不可区分 |
| 胜负 query 数 | ON 更好 **10** / OFF 更好 **11** / 打平 **10** ⇒ 完全均势 |
| 注入条数 | ON 均 10.07 / OFF 均 9.90，**逐 query 条数相同占 34/40** ⇒ 开关改变的是"**选哪几条**"，不是"注入几条"（出口受 `limit=max_memories` 封顶） |
| 共有 vs 差异条目均分 | **2.49 vs 1.96** ⇒ 差异发生在排序边缘，**头部相关条目两臂一致** |

**结论：非劣性通过 ⇒ 保留 ON。** 但要如实说：这次改动在**质量上是中性的**——既没测出提升，
也没测出退化（CI 跨 0、胜负均势）。保留它的理由是**确定性（可复现、可归因）+ 覆盖（近一半链接重新可达）**，
不是质量提升；"图谱通道该按什么排序才更好"仍是未解问题（本次只把"抽签"换成了"确定的规则"）。

**零足迹**：实验进程内 `TRACE_ENABLED=False` + 屏蔽 `recall_embedding_service.log`；用哨兵 query 核查
（3 次召回）⇒ `recall_traces` Δ0、`recall_embedding_logs` Δ0。另外本次工作期间**我自己**验证调用产生的
21 条 trace + 84 条 embedding 日志已清理（沿用 `recall_regression.py` 的既有约定：实验不得污染 trace 分析数据）。

## 未验证 / 边界（诚实清单）

- **质量只做到"非劣性"**：见上节 A/B —— Δ 与 0 不可区分，即"没有退化"也"没有提升"；
  "更好的排序规则"（例如按 mention_count 排、或按与 query 的语义距离排）仍未验证。
