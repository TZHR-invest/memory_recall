...
| **M2.1 claim 原子化** | [claim-atomicity](claim-atomicity.md) v1.1（生效） · [外部调研](../../notes/research/2026-08-19-claim-atomicity/README.md)（已完成） · [ADR-0020](../../decisions/0020-claim-atomicity.md)（Accepted） · [reconciliation-design](reconciliation-design.md) v2 · [entity-attributes](entity-attributes.md)（event_key/quoted_text 增量） | **P1/P2 文档定稿完成（2026-08-19）**：判据=独立生命周期 + 拆条=平行原子+event_key + 双上限隔离 + 文档类例外（长消息拆条/文档概括后置 P2）已定案；剩 P3 实现（拆条 + 存量重建，等用户确认） |
...
| 文件 | 层 | 角色 | 版本化 |
|------|----|------|--------|
| [foundation.md](foundation.md) | **语义** | 目标模型本体：北极星 + 对象模型 + 两链路 + 已拍板 39 项（#36–39 = M2.1 原子化定案）；**唯一裁判** | 独立版本（当前 v1 草稿） |
| [claim-atomicity.md](claim-atomicity.md) | 工程 | **M2.1 claim 原子化规范 + 迭代计划**（原子判据/拆条流程/event_key/evidence_quote/reinforce 边界/文档类例外/双上限/存量处理/验收） | 独立草稿，**已定稿（v1.1，2026-08-19）** |
| [entity-attributes.md](entity-attributes.md) | 落库 | crystal schema（evidence/claim/lineage_edge/claim_evidence/claim_usage/claim_activity 表字段/索引/枚举）**3 待定项已定案（2026-08-18）+ M2.1 event_key/quoted_text 增量** | 独立草稿，**已定稿**（M1 可照此建表） |
