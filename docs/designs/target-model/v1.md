...
**其余（Claim / Edge / Entity）全部是派生、可重新计算。**
...
- **Entity / entity_network（实体图谱扩展召回）**：旧系统的"实体关系 → 扩展召回候选 → 精排"，目的是提召回率，价值待验证。
  **关掉它核心照跑**，故不进核心底层设计；通过关系表反挂 Claim（**Claim 不主动关联 Entity**）。
- **主题（topic / category / tags）**：对无 scope 或跨项目记忆做"音乐/技术/…"分类，便于聚焦搜索；同为 P2 衍生能力，不做核心场景。
...
| `owner` | access control：个人 / 团队（v1 只个人） | 存储，继承采集上下文 |
| `embedding` | 检索用 | 存储 |
...
13. **owner = 个人/团队（access control，v1 只个人）**；砍掉"项目"作为 owner（项目是 scope，不是归属）；`owner×scope` 取代 S4 的 `owner×project + visibility`。
...
> 已拍板、不再重议的问题在末尾对照列出，避免重复讨论。

**待落设计文档（下一步要写的；写它们时把下面的拍板问题一并定掉）**：

1. 渐进迁移路径（Stage A：新 schema 与旧表并存、旧数据只读迁移；何时启动、迁移粒度）——收 C 档 13、A 档 2。
2. workbench 裁决界面（MR-011）：确认/纠错/遗忘/审计/审批的 API 与权限边界（用户纠正本身已定为特权 Evidence，见已拍板 #4）。
3. 「实体属性文档」：字段落库细节（类型、`status` 派生 vs 落库、`applicability` 是否拆出、scope/owner 的存储形态、Entity/主题 P2 的关系表设计）——收 C 档 14/15、A 档 1。

**拍板问题 A 档（进数据模型 / 写路径前必须定）**：

1. **画像（profile）在新模型里的地位**（收 MR-008/018）：「用户偏好/永久特征」是并入 Claim（`scope=无、owner=个人` 的聚合读视图，`memory_profiles` 变纯缓存或删），还是保留独立派生层？现倾向并入 Claim，待拍板。
2. **Evidence 采集可靠性**（收 MR-004）：写路径同步落库，还是持久化队列？Evidence 不可再生，内存异步队列进程退出即丢 = 永久丢失，故从 P2 升为 A 档。
3. **提炼判据（情景→语义，S1 心脏）**：判据是「有无具体时空/代词」还是「去掉上下文后仍为真且可复用」？v1 只挂了 `generalizes` 边名，没定「何时提炼」。
4. **知识库 vs 记忆系统边界 + 团队载体**（personal-vs-shared 拟采纳句未拍板）：「慢知识进知识库、快记忆进记忆、桥接单向（知识库→记忆）」是否成立；S4 团队知识落 git（docs）还是 db（共享表）？牵连 P2 文档蒸馏（MR-019）的定位。
5. **任务级上下文恢复是否 v2 一等场景**：用户痛点「做一件事回来要重新讲」vs v1 的「P3 全量理解不做」——「任务快照」这一档显式纳入，还是被「Evidence 重算」覆盖？

**拍板问题 B 档（S2/S3 阶段再定，不阻塞核心）**：

6. 衰减曲线形态 + 遗忘（forget）如何映射谱系边（retract 边？）。
7. 低置信召回策略：阈值截断 vs 降权排序；坚持不静默丢弃（MR-017 教训）。
8. 提炼/晋升触发主信号：意外冲突 vs 跨上下文重复 vs 用户显式「记住」（均缺可靠输入源）。
9. 安全门之投毒：四类失效 v1 只收了「失真=claim→claim 观察」，投毒未处理（可明确「暂不做」）。
10. LLM 自报信心初值修正规则：来源权重（`agent_add` / `outcome_trace` / `user_correction`）如何折算初值。
11. 团队多来源置信加权：「别人记的 vs 我记的」（v1 只个人，可明确推迟）。

**拍板问题 C 档（产品层 / 工程层，随迁移收尾）**：

12. **命名漂移（MR-010）**：晋升管线开工即 issue 预留的「重定名时点」——是否现在重定「提炼/成长」向新名（"Memory Recall" 只命名了记忆+召回，漏了灵魂与终点）。
13. 迁移框架（MR-013）→ 并入「渐进迁移路径」设计。
14. 注入 cap 契约（MR-017）→ 并入「召回路径（状态查询）」设计。
15. Entity 合并策略（MR-009）→ 做 Entity（P2）时定，非核心。
