...
| | **Lineage Edge（谱系边）** | Claim 之间"怎么演变"：`supersedes / generalizes / contradicts（临时）/ retract（单端）` + `reason` + `triggered_by_evidence` | **推理（"为什么这么变"）存在这条边上**，不塞进 Claim |
### 可选附属（P2，非核心）
...
**生命周期四阶段**：
1. **写入（对账）**：content 按 `source_kind` 给初值；复用/outcome = 0。
2. **召回（状态查询）**：结构化预过滤（scope + active）→ 向量粗排（top-K）→ 精排（相关性 × content × 复用·outcome）。
3. **消费（P1 回写）**：采用/忽略 + 结果好坏 → 复用频率 +1、outcome 计数（不碰 content）。
...
> **2026-08-15 更新：A 档 5 项已拍板（#16–#21）；二轮收敛新增 #22–#28（边类型/置信度单轴/砍有效区间/status 物化/召回顺序/删除恢复/处理状态机）。现剩 B 档（1–6）与 C 档（7–10）。**
...
3. 「实体属性文档」：**已出草稿**（[entity-attributes.md](entity-attributes.md)）；剩 `claim_kind` 取值、`evidence_refs` 数组vs关系表、`claim_usage` 落点待定——收 C 档 9（注入 cap）。Entity/主题 P2 → 另见「P2 实体网络文档」（收 C 档 10）。

**拍板问题 B 档（S2/S3 阶段再定，不阻塞核心）**：

1. 衰减曲线形态（content 时间衰减曲线 + 复用频率的时间窗口/半衰期）；遗忘映射 retract 边已定（#22）。
