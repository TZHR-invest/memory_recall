...
| M1 | `entity-attributes.md` 定稿（claim 3 待定项 + claim_usage 落点）+ **crystal API 契约 v1**（/api/v2 路由表、鉴权映射、错误规范） | `crystal.*` 建表 + `/api/v2` 骨架 | 空表可写、v5 零影响；schema 与 design 一致 |
| M2 | **workbench (MR-011) 设计 v1**（裁决面+洞察面 API/权限契约）+ **对账技术设计 v1**（worker/事务/retry）+ **召回技术设计 v1**（精排公式/索引/截断 + trace 契约）+ **crystal 测试策略** | 证据采集 + 对账 + 状态查询 + 裁决/洞察工作台 | 设计内验收标准逐条回填；测试矩阵绿 |
