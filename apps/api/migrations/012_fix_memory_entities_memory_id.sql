-- 修复 memory_entities 表中 memory_id 字段类型
-- 版本：012
-- 创建时间：2026-03-23
-- 说明：将 memory_entities 表的 memory_id 字段从 UUID 改为 VARCHAR(36)，以匹配 memories 表的 id 字段类型

-- ============================================================================
-- 修改 create_user_schema 函数
-- ============================================================================
...
BEGIN
    ...
    -- 创建 memory_entities 表（修复 memory_id 字段类型）
    EXECUTE format('
...
-- ============================================================================
-- 修复现有用户的 memory_entities 表
-- ============================================================================

CREATE OR REPLACE FUNCTION fix_memory_entities_memory_id_type()
RETURNS VOID AS $$
DECLARE
    user_record RECORD;
BEGIN
    FOR user_record IN SELECT schema_name FROM public.users LOOP
        BEGIN
            -- 删除旧的外键约束
            EXECUTE format('
                ALTER TABLE %I.memory_entities 
                DROP CONSTRAINT IF EXISTS fk_memory
            ', user_record.schema_name);
            
            -- 修改 memory_id 字段类型
            EXECUTE format('
                ALTER TABLE %I.memory_entities 
                ALTER COLUMN memory_id TYPE VARCHAR(36)
            ', user_record.schema_name);
            
            -- 重新创建外键约束
            EXECUTE format('
                ALTER TABLE %I.memory_entities 
                ADD CONSTRAINT fk_memory 
                FOREIGN KEY (memory_id) REFERENCES %I.memories(id) ON DELETE CASCADE
            ', user_record.schema_name, user_record.schema_name);
            
            RAISE NOTICE 'Fixed memory_entities table: %', user_record.schema_name;
        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE 'Skip table: %.memory_entities (error: %)', user_record.schema_name, SQLERRM;
        END;
    END LOOP;
END;
$$ LANGUAGE plpgsql;
...
-- 执行修复
SELECT fix_memory_entities_memory_id_type();
...
SELECT 'memory_entities.memory_id now supports VARCHAR(36)' AS info;
