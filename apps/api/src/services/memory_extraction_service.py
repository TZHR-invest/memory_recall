...
class MemoryExtractionService:
    ...
    async def extract_memories(
        ...
        try:
            ...
            if not memories:
                return {"success": False, "error": "未能提取到记忆", "memories": []}
    ...
    async def _call_llm_with_function_calling(self, content: str) -> Dict[str, Any]:
        """
        ...
        response = self.llm_client.call_with_tools(
            ...
            temperature=0.1,  # 降低温度以提高稳定性和一致性
    ...
    def _parse_json_response(self, response: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        从 JSON 响应中解析记忆（降级处理）
