...
async def _create_with_chunks(
    self, content: str, user_id: str, enable_graph: bool
) -> Dict[str, Any]:
    """
    ...
    unique_memories = all_memories  # 关闭去重
    print(
        f"📊 去重后：{len(unique_memories)} 个记忆点（去重 {len(all_memories) - len(unique_memories)} 个）"
    )
    ...
    for memory in unique_memories:
        ...
        # 从 entities 派生 location 和 people
        entities = memory.get("entities", [])
        location_entities = [e for e in entities if e.get("type") == "location"]
        location_name = location_entities[0].get("name") if location_entities else None
    ...
    return {
        ...
        "extracted": {"memories": len(unique_memories), "memory_ids": memory_ids},
    }


def _split_into_chunks(self, content: str, max_chars: int = 1000) -> List[str]:
    """
    将超长文本分块（按段落+大小+重叠窗口）
    ...
    for i, seg in enumerate(segments):
        ...
        for j, kept in enumerate(deduplicated):
            similarity = self._calculate_keyword_similarity(
                seg.get("_keywords", set()), kept.get("_keywords", set())
            )
            ...
            if similarity > 0.8:
                ...
                if seg_importance > kept_importance:
                    ...
                    deduplicated[j] = seg
                    print(
                        f"  🔄 替换重复记忆（相似度 {similarity:.2f}）：{kept.get('content', '')[:30]}... → {seg.get('content', '')[:30]}..."
                    )
                else:
                    print(
                        f"  🗑️ 跳过重复记忆（相似度 {similarity:.2f}）：{seg.get('content', '')[:30]}..."
                    )
...
def _deduplicate_memories_by_content(
    ...
    for i, memory in enumerate(memories):
        ...
        for j, kept in enumerate(deduplicated):
            similarity = self._calculate_keyword_similarity(
                memory.get("_keywords", set()), kept.get("_keywords", set())
            )
            ...
            if similarity > 0.8:
                ...
                if memory_importance > kept_importance:
                    ...
                    print(
                        f"  🔄 替换重复记忆（相似度 {similarity:.2f}）：{kept.get('content', '')[:30]}... → {memory.get('content', '')[:30]}..."
                    )
                else:
                    print(
                        f"  🗑️ 跳过重复记忆（相似度 {similarity:.2f}）：{memory.get('content', '')[:30]}..."
                    )
