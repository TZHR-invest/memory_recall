...
class EntityDictionaryService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            for user in users:
                ...
                try:
                    ...
                    async with db.user_context(user_id):
                        ...
                        # 2.2 加载 same_as 关系（别称映射）
                        same_as_relations = await db.fetch(
                            """
                            SELECT 
                                e1.name as entity_name,
                                e2.name as alias_name,
                                r.from_entity_id,
                                r.to_entity_id
                            FROM relations r
                        ...
                        for rel in same_as_relations:
                            ...
                            # 双向映射：两个实体互为别名
                            if from_id not in entity_id_to_aliases:
                                entity_id_to_aliases[from_id] = []
                            if to_id not in entity_id_to_aliases:
                                entity_id_to_aliases[to_id] = []

                            entity_id_to_aliases[from_id].append(alias_name)
                            entity_id_to_aliases[to_id].append(entity_name)

                            # 别称映射到规范实体
                            alias_to_entity[entity_name] = {
                                "canonical_name": alias_name,
                                "canonical_id": to_id,
                            }
                            alias_to_entity[alias_name] = {
                        ...
                        for entity in entities:
                            ...
                            entity_info = {
                                ...
                                "aliases": entity_id_to_aliases.get(entity_id, []),
                            }
                        ...
                        # 2.5 添加别称映射
                        for alias_name, alias_info in alias_to_entity.items():
                            if alias_name not in self.entity_dict:
                                self.entity_dict[alias_name] = alias_info
            ...
            logger.info(
                f"实体词典初始化完成，共加载 {len(self.entity_dict)} 个实体（来自 {len(users)} 个用户）"
            )
    ...
    def extract_entities_fast(
        ...
        sorted_names = sorted(
            self.entity_dict.keys(), key=lambda x: len(x), reverse=True
        )
        ...
        for name in sorted_names:
            if name in query:
                ...
                # 如果是别称，获取规范实体名
                canonical_name = name
                if isinstance(entity_info, dict) and "canonical_name" in entity_info:
                    canonical_name = entity_info["canonical_name"]
                    entity_info = self.entity_dict.get(canonical_name, entity_info)
                ...
                if user_id:
                    if isinstance(entity_info, list):
                        ...
                        if (
                            ...
                            and entity_info.get("user_id") == user_id
                        ):
                            entities.append(canonical_name)
                else:
                    # 未指定 user_id，直接添加
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sample_entities": list(self.entity_dict.keys())[:10]
            if self.entity_dict
            else [],
        }
