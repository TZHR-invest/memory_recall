...
class MemoryService:
    ...
    async def create(self, memory_data: MemoryCreate) -> str:
        ...
        """,
            ...
            json.dumps([p.model_dump() for p in memory_data.people])
            if memory_data.people
            else None,
            json.dumps(memory_data.emotion.model_dump())
            if memory_data.emotion
            else None,
            json.dumps(memory_data.tags) if memory_data.tags else None,
            json.dumps(memory_data.duration.model_dump())
            if memory_data.duration
            else None,
            json.dumps(memory_data.topic.model_dump()) if memory_data.topic else None,
            json.dumps([a.model_dump() for a in memory_data.attachments])
            if memory_data.attachments
            else None,
            # 向量数据：转换为字符串格式
            "[" + ",".join(map(str, memory_data.embedding)) + "]"
    ...
    async def create_memory_with_graph_v2(
        ...
        MAX_CHARS_PER_CHUNK = 5000  # 超过 5000 字符才分段（测试用）
        ...
        if not extraction_result or not extraction_result.get("success"):
            ...
            return {
                ...
                "extracted": {"memory_ids": [memory_id], "count": 1},
            }
        ...
        async with db.transaction(user_id) as conn:
            for memory in memories:
                ...
                # 从 entities 中派生 location 和 people
                location_entities = [e for e in entities if e.get("type") == "location"]
                location_name = (
                    location_entities[0].get("name") if location_entities else None
                )

                person_entities = [e for e in entities if e.get("type") == "person"]
                people = [{"name": e.get("name")} for e in person_entities]
        ...
        return {
            ...
            "extracted": {"memories": len(memories), "memory_ids": memory_ids},
        }
    ...
    async def _store_graph_v2(
        ...
        for relation in relations:
            ...
            print(f"🔍 存储关系: {source} --{relation_type}--> {target}")

            # ⚠️ 检查 target 不能是"我"
            if target == "我":
                print(f"⚠️ 跳过：target 是'我'")
                continue
    ...
    async def _upsert_relation_v2(
        ...
        try:
            ...
            if not relation_type:
                print(
                    f"⚠️ 跳过无效关系：relation_type 为空 (source={source}, target={target})"
                )
                return False
    ...
    async def _store_memory(
        self, content: str, embedding: Optional[List[float]], user_id: str
    ) -> str:
        """
        存储记忆到数据库
    ...
    async def batch_create_memories(
        self, contents: List[str], user_id: str, enable_graph: bool = True
    ) -> Dict[str, Any]:
        """
        批量创建记忆（减少数据库连接次数）
        ...
        # 3. 并发构建图谱
        graph_results = []
        if enable_graph and contents:
            graph_builder = get_graph_builder_service()

            tasks = [
                graph_builder.build_graph(
                    content=content, user_id=user_id, enable_confirmation=False
                )
                for content in contents
            ]
    ...
    async def create_memory_from_file(
        ...
        Args:
            ...
            user_id: 用户 ID
            enable_graph: 是否启用图谱构建（默认 True）
            enable_dedup: 是否启用文件去重检查（默认 True，暂未实现）
        ...
        print(f"📁 处理文件: {file_name} (大小: {file_size} 字节)")
        ...
        try:
            ...
            try:
                ...
                return {
                    "status": "error",
                    "message": "无法解析文件编码，请使用 UTF-8 或 GBK 编码",
                }

        # 2. 调用记忆创建方法
        result = await self.create_memory_with_graph_v2(
            content=text_content, user_id=user_id, enable_graph=enable_graph
        )
        ...
        if not memory_ids:
            return {"status": "error", "message": "记忆创建失败"}
