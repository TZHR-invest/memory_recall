...
class GraphEnhancedRecallService:
            ...
            return [
                {
                    ...
                    "people": m["people"],
                    "entity": {"name": entity_name, "type": None, "confidence": None},
                    "mention_context": m["mention_context"],
                    "similarity": 0.9,  # 图谱召回的相似度（实体精确匹配）
                    "recall_type": "graph_entity",
                }
                for m in memories
            ]
            ...
            return [
                {
                    ...
                    "relations": [
                        ...
                        for r in relations
                        if r["source"] in m["entities"]
                        or r["destination"] in m["entities"]
                    ],
                    "similarity": 0.85,  # 图谱召回的相似度（关系匹配）
                    "recall_type": "graph_relation",
                }
                for m in memories
            ]
    ...
    async def search_graph(
        ...
        try:
            # 1. 提取实体（使用词典匹配）
            entities = await self._extract_entities_from_query(
                query, user_id, use_dict=True
            )
            ...
            expanded_entity_names = await self._expand_entities_by_normalization(
                [e["name"] for e in entity_results], user_id
            )
            ...
            relations = await self._get_entity_relations(
                [e["id"] for e in entity_results], user_id
            )
            ...
            memories = await self._get_memories_by_entities(
                list(entity_names), user_id, limit, time_range
            )

            return {"relations": reranked_relations[:limit], "memories": memories}
    ...
    async def _extract_entities_from_query(
        ...
        Args:
            ...
            enhanced_mode: 增强模式
                - "exact": 仅精确匹配（最快）
                - "keyword": 精确 + 关键词匹配（推荐）
                - "enhanced": 精确 + 关键词 + 模糊匹配
                - "full": 精确 + 关键词 + 模糊 + 语义匹配（最慢但最强）
                - "auto": 自动选择（先精确，未找到则关键词，再未找到则模糊）
        ...
        if use_dict:
            ...
            try:
                ...
                logger.info(
                    f"词典实体提取耗时: {elapsed_ms:.2f}ms, 提取到 {len(entities)} 个实体"
                )
                ...
                if not entities and enhanced_mode != "exact":
                    ...
                    if enhanced_entities:
                        ...
                        logger.info(f"增强匹配找到 {len(entities)} 个实体")
    ...
    async def _enhanced_entity_extraction(
        self, query: str, user_id: str, mode: str = "keyword"
    ) -> List[tuple]:
        """
        增强的实体提取（解决精确匹配局限性）
        ...
        Args:
            query: 查询文本
            user_id: 用户 ID
            mode: 增强模式（keyword/enhanced/full）

        Returns:
            [(实体名称, 匹配方法, 置信度), ...]
        """
        try:
            from .enhanced_entity_extractor import get_enhanced_entity_extractor
            ...
            if not extractor._initialized:
                # 同步实体词典
                extractor.entity_dict = self.entity_dict.entity_dict
            ...
            if mode == "keyword":
                ...
                methods = ["keyword", "fuzzy"]
            elif mode == "full":
                methods = ["keyword", "fuzzy", "semantic"]
            else:  # auto
                methods = ["keyword"]
    ...
    async def _extract_entities_with_llm(self, query: str) -> List[str]:
        ...
        try:
            ...
            if response.get("tool_calls"):
                entities = response["tool_calls"][0]["function"]["arguments"][
                    "entities"
                ]
    ...
    async def _get_entity_relations(
        self, entity_ids: List[str], user_id: str
    ) -> List[Dict]:
        """获取实体的所有关系（包括归一化关系）"""
    ...
    def _bm25_rerank(self, query: str, relations: List[Dict]) -> List[Dict]:
        """
        ...
        try:
            ...
            for r in relations:
                # 将英文关系类型转换成中文（如果存在映射）
                relationship_cn = RELATION_TYPES.get(
            ...
            top_indices = bm25.get_top_n(
                tokenized_query, list(range(len(documents))), n=len(documents)
            )
    ...
    def _merge_and_rank(
        ...
        # 排序
        sorted_ids = sorted(
            memory_scores.keys(), key=lambda x: memory_scores[x], reverse=True
        )

        # 返回结果，更新最终的 similarity 为综合分数
        results = []
