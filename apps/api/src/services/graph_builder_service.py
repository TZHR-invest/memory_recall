...
class GraphBuilderService:
    ...
    async def build_graph(
        ...
        try:
            ...
            # 检查是否需要确认
            if isinstance(entities_result, dict) and entities_result.get(
                "need_confirm"
            ):
                return {
                    "entities": [],
            ...
            if response.get("tool_calls"):
                for tool_call in response["tool_calls"]:
                    if tool_call["function"]["name"] == "extract_entities":
                        entities = tool_call["function"]["arguments"].get(
                            "entities", []
                        )
                        # 确保每个实体都有置信度
                        for entity in entities:
    ...
    async def _find_similar_entities(self, entity_name: str, user_id: str) -> List[str]:
        """
        查找相似实体（用于智能询问）
            ...
            if response.get("tool_calls"):
                for tool_call in response["tool_calls"]:
                    if tool_call["function"]["name"] == "establish_relations":
                        relations = tool_call["function"]["arguments"].get(
                            "relations", []
                        )
                        # 确保每个关系都有置信度
                        for relation in relations:
    ...
    async def _create_memory_entity_link(
        self, memory_id: str, entity_id: str, mention_context: Optional[str] = None
    ) -> bool:
        """
        创建记忆-实体关联
