...
export function getAiGuidance(isZh: boolean): string[] {
  return isZh 
    ? [
        ...
        "",
        "### Step 1: 检查已注入上下文",
        "- Session Summary 是否包含相关信息？",
        "- Project Memories 是否包含相关决策？",
        "- Project Documents 是否包含相关文档？",
        "",
        "### Step 2: 判断是否需要召回",
        "- ✅ 如果注入上下文已包含答案 → **直接回答，禁止探索**",
        "- ⚠️ 如果注入上下文部分包含 → 补充召回",
        "- ❌ 如果注入上下文完全不相关 → 执行探索",
        "",
        "### Step 3: 决定召回策略",
        "- 问题涉及历史信息 → 启用 Memory Graph (`enableMemoryGraph: true`)",
        "- 问题涉及实体关系 → 启用 Entity Graph (`enableEntityGraph: true`)",
        "- 问题涉及文档内容 → 搜索文档 (`memory-recall(mode: \"search\", query: \"...\")`)",
        "",
        "**召回场景与参数选择**：",
        "| 场景 | 参数 | 说明 |",
        "|------|------|------|",
        ...
        "- 探索代码库平均耗时：30-60 秒",
        "- 上下文召回平均耗时：< 1 秒",
        "- 请优先使用上下文，只有在上下文确实不足时再探索",
        "",
      ]
    : [
        "### 🚨 AI Behavior Guidance (Mandatory)",
        "",
        "**[MANDATORY RULES] Before exploring the codebase, you MUST:**",
        "",
        "### Step 1: Check Injected Context",
        "- Does Session Summary contain relevant information?",
        "- Do Project Memories contain relevant decisions?",
        "- Do Project Documents contain relevant documentation?",
        "",
        "### Step 2: Determine if Recall is Needed",
        "- ✅ If injected context contains the answer → **Answer directly, DO NOT explore**",
        "- ⚠️ If injected context partially contains → Supplement with recall",
        "- ❌ If injected context is completely irrelevant → Proceed with exploration",
        "",
        "### Step 3: Decide Recall Strategy",
        "- Question involves historical info → Enable Memory Graph (`enableMemoryGraph: true`)",
        "- Question involves entity relations → Enable Entity Graph (`enableEntityGraph: true`)",
        "- Question involves document content → Search documents (`memory-recall(mode: \"search\", query: \"...\")`)",
        "",
        ...
        "",
        "**[Efficiency Tips]**:",
        "- Average codebase exploration time: 30-60 seconds",
        "- Average context recall time: < 1 second",
        "- Prioritize using context, explore only when context is truly insufficient",
        "",
      ];
}
